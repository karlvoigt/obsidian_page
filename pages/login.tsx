...
export default function Login() {
  ...
  const handleLogin = async (provider: 'google' | 'github' | 'microsoft') => {
    ...
    try {
      ...
      // Error is handled in context, but we catch to stop loading
      console.error(err);
    } finally {
      setLoggingIn(null);
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-tr from-slate-900 via-slate-800 to-indigo-950 flex flex-col justify-center items-center px-4 py-12">
      <Head>
        <title>Sign In - Vault Notes</title>
      </Head>
      ...
      <div className="w-full max-w-md bg-white/5 backdrop-blur-lg border border-white/10 shadow-2xl rounded-2xl p-8 md:p-10 text-center">
        <div className="mb-8">
          <div className="h-12 w-12 bg-blue-600 rounded-xl flex items-center justify-center mx-auto mb-4 shadow-lg shadow-blue-500/30">
            <span className="text-white text-2xl font-bold">V</span>
          </div>
          <h2 className="text-3xl font-extrabold text-white tracking-tight">Access Locked</h2>
          <p className="mt-2 text-sm text-gray-300">
            This repository is private. Please sign in to verify your access.
          </p>
        </div>
